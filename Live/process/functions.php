...
*/    
    ...
    private function get_livestream_info(){
        $url = "https://api.aparat.com/fa/v2/live/live/get_stream/username/Taziyekhani";
        /*
        $data = json_decode($contents, true);
        
        $contents=file_get_contents($url);
        $live_status = $authToken;
        ...
        */
        ...
        $ccc = curl_exec($curlSession);
        $jsonData = json_decode($ccc);
        curl_close($curlSession);
        
        
        return array('live_status'=>null, 'stream_line'=>$stream_line);
    }
    
    
    public function get_comments(){
        $cid = $this->channel_id();
        $comments=$this->FetchData($this->comments,"name,text","cid='$cid' and confirm=1 ORDER BY `date` ASC");
        return $comments;
    }
